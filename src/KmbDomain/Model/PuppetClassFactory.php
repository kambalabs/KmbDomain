...
class PuppetClassFactory implements PuppetClassFactoryInterface
{
    /**
     * @param string         $name
    ...
    public function create($name, $group, $parametersTemplates = null)
    {
        ...
        $class->setParameters($this->createParameters($parametersTemplates));
        return $class;
    }
     ...
     * @return Parameter[]
     */
    protected function createParameters($parametersTemplates)
    {
        $parameters = [];
        if (!empty($parametersTemplates)) {
            foreach ($parametersTemplates as $parameterTemplate) {
